This is a test for the open source rendering engine OpenWeb.
...
If you would like to build, test or modify OpenWeb, please be aware of some prerequisites. OpenWeb currently uses SFML for its graphics handling in addition to other libraries, including the "C String Toolkit Library" (which in turn relies on Boost). Both of these libraries are necessary to build OpenWeb. Finally, one last major current shortcoming is the implementation of fonts. As of 3/25/13, OpenWeb relies on a directory within the executable directory called "fonts" that contains the DejaVuSans True Type Font.

If nothing else, OpenWeb has taught me quite a bit about how to use external libraries. I spent a weekend fighting various graphics libraries, trying to force them to compile and link with OpenWeb properly--and, at the end of it, I have learned a significant amount about libraries, compilation, linking, and how UNIX works in general.
