...
If you would like to build, test or modify OpenWeb, please be aware of some prerequisites. OpenWeb currently uses SFML for its graphics handling, and will have SFGUI support added very soon. These libraries are necessary to build OpenWeb. Additionally, other libraries, including the "C String Toolkit Library" (which in turn relies on Boost), are used. Finally, one last major current shortcoming is the implementation of fonts. As of 3/13/13, OpenWeb relies on a directory within the executable directory called "fonts" that contains the Linux Libertine fonts (which I highly recommend). This dependency will be fixed in the future--this is, remember, a prototype.
